...
COLD void initVfo(void)
{
        ...
        //#define OCXO_10MHZ in main header file - Uncomment it there for external reference on the si5351C board version
            ...
            */
            ...
            si5351.set_pll(SI5351_PLL_FIXED, SI5351_PLLB); //also make PPLB have an input
            si5351.drive_strength(SI5351_CLK0, SI5351_DRIVE_2MA); // note these were 8MA
            si5351.drive_strength(SI5351_CLK1, SI5351_DRIVE_2MA);
            si5351.drive_strength(SI5351_CLK2, SI5351_DRIVE_2MA);
            si5351.set_ms_source (SI5351_CLK2, SI5351_PLLB); // CLK2 uses PLLB so it won't move around as we move PLLA
            currentEvenDivisor = calcDivisor(VFO_A);
            si5351.set_freq_manual(VFO_A * SI5351_FREQ_MULT, currentEvenDivisor * VFO_A * SI5351_FREQ_MULT, SI5351_CLK0);

            si5351.set_freq_manual(VFO_A * SI5351_FREQ_MULT, currentEvenDivisor * VFO_A * SI5351_FREQ_MULT, SI5351_CLK1);
            si5351.set_phase(SI5351_CLK0, 0);
            si5351.set_phase(SI5351_CLK1, currentEvenDivisor);
...
int oldEvenDivisor = currentEvenDivisor;
  currentEvenDivisor = calcDivisor(Freq);
  si5351.set_freq_manual(Freq * SI5351_FREQ_MULT, currentEvenDivisor * Freq * SI5351_FREQ_MULT, SI5351_CLK0);

  si5351.set_freq_manual(Freq * SI5351_FREQ_MULT, currentEvenDivisor * Freq * SI5351_FREQ_MULT, SI5351_CLK1);
  si5351.set_phase(SI5351_CLK0, 0);
  si5351.set_phase(SI5351_CLK1, currentEvenDivisor);
  if (currentEvenDivisor != oldEvenDivisor) {   // Reset of the SI5351 is needed only after a change in divisor
    si5351.pll_reset(SI5351_PLLA);
  }
#else // VFO_MULT != 1
    #ifdef USE_RS_HFIQ
