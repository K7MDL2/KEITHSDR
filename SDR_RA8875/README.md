...
## April 2022
    ...
    14. (April 24) reworked (again) the VFOA/B logic. Changed the concept of how VFOB is handled. Both VFOs were coupled together per band. VFOB is now uncoupled led from VFOA and can be on any band and has no per-band settings other than mode and relations to mode (which is under dev). Thsi is also like the K3 and rther radios. Split works. This better supports CAT port commands for things like VFO A and B updates, swapping VFOs, and reduces code line count and code complexity.  Tested only with the RS-HFIQ so far, should work with other RF hardware OK. Be sure to install udpated (as of April 24/25) Teensy_SDR_RS-HFIQ library.  If using Omni-Rig interface install the updated Omni-Rig to match changes. This is an evolving area so expect more changes for some time in this area.
