...
## May 2022
    ...
    16. (May 15) - Corrected FFT sideband display in TX mode.  Set default mic gain to 64% which is below the clipping indicator with my Beofang speaker mic connected.  USB and LSB now work as does the two tone test when the ATU button is ON. BUG: Occasionally transition frm TX to RX cause audio chain to stop, a reset is required.  I have the mic PTT wired to the RS-HFIQ paddle jack ring terminal with does put the hardware into TX, but the Teensy does not know it. Will probably move the mic PTT wire to GPIO for debounce and let the Teensy control PTT and also can act as a keyer down the road, or at least uses the mic PTT line as a code key input line while in CW mode.
