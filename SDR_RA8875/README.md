...
## Jan 2023
    ...
    12. Jan 18 - Added support for Transverters up to 47GHz. The bandmem table was expanded to 23 predefined ham bands to support all the per-band features. This required moving to uint64_t for all frequency related variables and a lot of files were touched in this this round.  Now CAT commands for frequency pass through to the main program where it is validated or dropped.  Before replies and band validation was performend in the RS-HFIQ file.
    13. The BandMem table in SDR_Data.h was expanded for Xvtr support.  A new BandMap field was added also.  0 disables the band causing band up/down to skip over them, and for CAT commands to ignore them.  Sicne most Xvtr bands are unused by most folks this feature was required sooner than later.  This will also be the mechanism to limit bands to particular hardware such as the RS-HFIQ which only supports 80-10M, and which buttons will display in teh Band Menu window.
    14. Transverter bands are exactly the same as regular bands except they will have some extended features linked in a Xvtr table for power control and other Xvtr-only features.  Transverters convert one band to another so the displayed frequency must have an offset representing the difference between the IF and RF frequencies.  The radio hardware tuning must be on a fixed band such as 28MHz - the IF frequency. A new IF field is set to 0 for non-Xvtr bands, and set to a band index value such as BAND10M or BAND15M to use 10M or 15M bands as the IF band. The program will automatically calculate the offset required.  Bands higher than 47G are possible to read out direct but some VFO dispay formatting work needs to be done, or you can try leaving off the left most digit(s).  
    15. The VFO number display area needs to be formatted, the larger numbers are not positions properly yet.
