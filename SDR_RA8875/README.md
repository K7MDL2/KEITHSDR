...
## Feb 2022
    ...
    34. Invesdtigating fixes for "Twin Peaks" codec sync. Goal is to detect and automate the fix. Nost of initDSP() has been moved to a new resetcodec() that can be called during runtime also to restart the audio chain and attempt to remove the image. For now if a mirror image is seen press the XMIT button which for now calls resetcodec() on return to receive mode. You may need to do this a few times but eventually it does work.  Various teat programs have been created to detect this condition in the SGTL5000 codec startup. Looking at ways to integrate them.
