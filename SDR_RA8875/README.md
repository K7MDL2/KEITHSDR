...
## Jan 2023
    ...
    9. Added continuously variable audio filter when operated from an encoder.  Tapping the Filter button still cycles through predefined list of bandwidths as before.  Filter label format changed to always display as X.XXKHz. Under 1KHz steps in 50Hz widths, > 1K 100Hz and > 3K 200Hz. Max is 6K. MInis 150Hz for non CW modes, 50Hz for CW modes.  FM mode will have N/A for filter and is fixed bandwidth (and does nto work anyway yet but the screen is right).  The variable will usethe last button filter setting and increment smoothly off that value.  The opposite is not true yet.  Change the filter and the button tap does not yet know about it. Fix in the queue.  The RGB knobs will turn red at filter range ends also.
