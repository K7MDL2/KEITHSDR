...
## May 2022
    ...
    16. (May 15) - Corrected FFT sideband display in TX mode.  Set default mic gain to 76% which is below the clipping indicator with my old Raio Shack dynamic hand mic connected.  USB and LSB now work as does the Two-Tone Test when the ATU button is ON. BUG: Occasionally transition from TX to RX causes the audio chain to stop somewhere after the FFT, a CPU reset is required. I have the mic PTT wired to the RS-HFIQ paddle jack ring terminal which puts the hardware into TX, but the Teensy does not know it. Will probably move the mic PTT wire to GPIO for debounce and let the Teensy control the RS-HFIQ TX mode and can act as a keyer down the road, or at least uses the mic PTT line as a code key input line while in CW mode.  New TX-only filters added, with center and width set in the 2 variables 'TX_filterCenter' and 'TX_filterBandwidth' which are initialized in the upper section of the .ino file for now.
