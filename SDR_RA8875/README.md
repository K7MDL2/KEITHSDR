...
## Feb 2022
    ...
    36.  Added code ( PhaseChange(x) ) to remove the 1 bit delay in I2S audio input that often happens on startup, aka "Twin Peaks" or mirror FFT image problem. It is assigned to the ANT button (Long press - will beep on release). Removed resetcodec() from the Xmit button. Tap and hold the ANT button until the mirror image goes away. Investigating whay AudioSDRpreProcessor won't compile for me.  It has auto-detection logic I would prefer to use.
